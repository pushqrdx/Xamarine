...
namespace Xamarine.Hosting.Modules
{
    ...
    public abstract class FileModuleBase : WebModuleBase
    {
        /// <summary>
        ...
        private readonly Lazy<Dictionary<string, string>> _mimeTypes = new Lazy<Dictionary<string, string>>(() => new Dictionary<string, string>(Constants.MimeTypes.DefaultMimeTypes, Strings.StandardStringComparer));
        ...
        ///     The MIME types.
        /// </value>
        public Lazy<ReadOnlyDictionary<string, string>> MimeTypes => new Lazy<ReadOnlyDictionary<string, string>>(() => new ReadOnlyDictionary<string, string>(_mimeTypes.Value));
        ...
        protected Task WriteFileAsync(bool usingPartial, string partialHeader, long fileSize, HttpListenerContext context, Stream buffer, CancellationToken ct)
        {
            long lowerByteIndex = 0;

            if (usingPartial && CalculateRange(partialHeader, fileSize, out lowerByteIndex, out var upperByteIndex))
            {
                ...
                else
                {
                    ...
                    context.Response.AddHeader(Headers.ContentRanges, $"bytes {lowerByteIndex}-{upperByteIndex}/{fileSize}");
                }
            }
            else
            {
                if (UseGzip && context.RequestHeader(Headers.AcceptEncoding).Contains(Headers.CompressionGzip) && buffer.Length < MaxGzipInputLength &&

                    // Ignore audio/video from compression
                    context.Response.ContentType?.StartsWith("audio") == false && context.Response.ContentType?.StartsWith("video") == false)
                {
        ...
        {
            response.AddHeader(Headers.CacheControl, DefaultHeaders.GetValueOrDefault(Headers.CacheControl, "private"));
        ...
        private static async Task WriteToOutputStream(HttpListenerResponse response, Stream buffer, long lowerByteIndex, CancellationToken ct)
        {
            ...
            while (true)
            {
                if (sendData + ChunkSize > response.ContentLength64)
                    readBufferSize = (int) (response.ContentLength64 - sendData);
        ...
        private static bool CalculateRange(string partialHeader, long fileSize, out long lowerByteIndex, out long upperByteIndex)
        {
            ...
            if (range.Length == 2 && long.TryParse(range[0], out lowerByteIndex) && long.TryParse(range[1], out upperByteIndex))
                return true;

            if (range.Length == 2 && long.TryParse(range[0], out lowerByteIndex) && string.IsNullOrWhiteSpace(range[1]) || range.Length == 1 && long.TryParse(range[0], out lowerByteIndex))
            {
                upperByteIndex = (int) fileSize;
                return true;
            }

            if (range.Length == 2 && string.IsNullOrWhiteSpace(range[0]) && long.TryParse(range[1], out upperByteIndex))
            {
