...
using Xamarine.Hosting.Swan;
using HttpListenerContext = Xamarine.Hosting.Network.HttpListenerContext;
using HttpListenerException = Xamarine.Hosting.Network.HttpListenerException;
using HttpListenerResponse = Xamarine.Hosting.Network.HttpListenerResponse;
...
namespace Xamarine.Hosting.Modules
{
    ...
    public class StaticFilesModule : FileModuleBase
    {
        ...
        public StaticFilesModule(string fileSystemPath, Dictionary<string, string> headers = null, Dictionary<string, string> additionalPaths = null, bool useDirectoryBrowser = false)
        {
...
// Otherwise, enable it by default
            ...
            additionalPaths?.Where(path => path.Key != "/").ToDictionary(x => x.Key, x => x.Value).ForEach(RegisterVirtualPath);
        ...
        {
            _virtualPaths.UnregisterVirtualPath(virtualPath);
        }
        ...
        {
            var entries = new[] {context.Request.RawUrl == "/" ? string.Empty : "<a href='../'>../</a>"}.Concat(Directory.GetDirectories(localPath).Select(path =>
            {
            ...
            }).OrderBy(x => x.Name).Union(Directory.GetFiles(localPath, "*", SearchOption.TopDirectoryOnly).Select(path =>
            {
                var fileInfo = new FileInfo(path);
                var name = Path.GetFileName(path);

                return new {Name = name.Truncate(MaxEntryLength, "..>"), Url = Uri.EscapeDataString(name), ModificationTime = fileInfo.LastWriteTimeUtc, Size = fileInfo.Length.FormatBytes()};
            }).OrderBy(x => x.Name)).Select(y => $"<a href='{y.Url}'>{WebUtility.HtmlEncode(y.Name)}</a>" + new string(' ', MaxEntryLength - y.Name.Length + 1) +
                                                 y.ModificationTime.ToString(Strings.BrowserTimeFormat, CultureInfo.InvariantCulture) + new string(' ', SizeIndent - y.Size.Length) + y.Size)).Where(x => !string.IsNullOrWhiteSpace(x));

            var content = Responses.ResponseBaseHtml.Replace("{0}", $"<h1>Index of {WebUtility.HtmlEncode(context.RequestPathCaseSensitive())}</h1><hr/><pre>{string.Join("\n", entries)}</pre><hr/>");
        ...
        private Task<bool> HandleGet(HttpListenerContext context, CancellationToken ct, bool sendBuffer = true)
        {
            ...
            {
                case Modules.VirtualPaths.VirtualPathStatus.Forbidden:
                    context.Response.StatusCode = (int) HttpStatusCode.Forbidden;
                    return Task.FromResult(true);
                case Modules.VirtualPaths.VirtualPathStatus.File:
                    return HandleFile(context, requestFullLocalPath, sendBuffer, ct);
                case Modules.VirtualPaths.VirtualPathStatus.Directory:
                ...
                else
                {
                    ...
                    if (sendBuffer)
                    {
                        ...
                        if (usingPartial == false)
                            isTagValid = UpdateFileCache(context.Response, buffer, fileDate, context.RequestHeader(Headers.IfNotMatch), localPath);
                    }
                }

                // check to see if the file was modified or e-tag is the same
                var utcFileDateString = fileDate.ToUniversalTime().ToString(Strings.BrowserTimeFormat, Strings.StandardCultureInfo);

                if (usingPartial == false && (isTagValid || context.RequestHeader(Headers.IfModifiedSince).Equals(utcFileDateString)))
                {
        ...
        private VirtualPaths.VirtualPathStatus ValidatePath(HttpListenerContext context, out string requestFullLocalPath)
        {
        ...
        private bool UpdateFileCache(HttpListenerResponse response, Stream buffer, DateTime fileDate, string requestHash, string localPath)
        {
