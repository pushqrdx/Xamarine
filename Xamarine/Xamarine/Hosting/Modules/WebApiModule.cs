...
namespace Xamarine.Hosting.Modules
{
    ...
    ///     Public instance methods that match the WebServerModule.ResponseHandler signature, and have the WebApi handler
    ///     attribute
    ...
    public class WebApiModule : WebModuleBase
    {
        ...
        public WebApiModule()
        {
            AddHandler(ModuleMap.AnyPath, HttpVerbs.Any, async (context, ct) =>
            {
                ...
                var path = Server.RoutingStrategy == RoutingStrategy.Wildcard ? NormalizeWildcardPath(verb, context) : NormalizeRegexPath(verb, context, regExRouteParams);
                ...
                $"Handler: {methodPair.MethodCache.ControllerName}.{methodPair.MethodCache.MethodInfo.Name}".Debug(nameof(WebApiModule));
        ...
        public void RegisterController<T>() where T : WebApiController, new()
        {
        ...
        /// <exception cref="System.ArgumentException">Controller types must be unique within the module.</exception>
        public void RegisterController<T>(Func<T> controllerFactory) where T : WebApiController
        {
        ...
        public void RegisterController(Type controllerType, Func<object> controllerFactory)
        {
            ...
            var methods = controllerType.GetMethods(BindingFlags.Instance | BindingFlags.Public).Where(m => (m.ReturnType == protoDelegate.GetMethodInfo().ReturnType || m.ReturnType == protoAsyncDelegate.GetMethodInfo().ReturnType) &&
                                                                                                            m.GetParameters().Select(pi => pi.ParameterType).Take(2)
                                                                                                             .SequenceEqual(protoDelegate.GetMethodInfo().GetParameters().Select(pi => pi.ParameterType)));
        ...
        private string NormalizeRegexPath(HttpVerbs verb, HttpListenerContext context, Dictionary<string, object> routeParams)
        {
        ...
        {
            var path = context.RequestWilcardPath(_delegateMap.Keys.Where(k => k.Contains(ModuleMap.AnyPathRoute)).Select(s => s.ToLowerInvariant()));
            ...
            if (_delegateMap.ContainsKey(originalPath) && _delegateMap[originalPath].ContainsKey(verb))
                return originalPath;
        ...
        private bool IsMethodNotAllowed(HttpListenerContext context)
        {
            ...
            switch (Server.RoutingStrategy)
            {
                case RoutingStrategy.Wildcard:
                    path = context.RequestWilcardPath(_delegateMap.Keys.Where(k => k.Contains("/" + ModuleMap.AnyPath)).Select(s => s.ToLowerInvariant()));
                    break;
                case RoutingStrategy.Regex:
...
#region Immutable Declarations
        ...
        private readonly Dictionary<string, Dictionary<HttpVerbs, MethodCacheInstance>> _delegateMap = new Dictionary<string, Dictionary<HttpVerbs, MethodCacheInstance>>(Strings.StandardStringComparer);
    ...
    public abstract class WebApiController
    {
        ...
        /// </summary>
        /// <param name="context">The context.</param>
        public virtual void SetDefaultHeaders(HttpListenerContext context)
        {
            context.NoCache();
        }
    }
