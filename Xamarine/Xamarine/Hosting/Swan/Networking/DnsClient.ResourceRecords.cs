...
namespace Xamarine.Hosting.Swan.Networking
{
    ...
    internal partial class DnsClient
    {
        public abstract class DnsResourceRecordBase : IDnsResourceRecord
        {
            ...
            protected virtual string[] IncludedProperties => new[] {nameof(Name), nameof(Type), nameof(Class), nameof(TimeToLive), nameof(DataLength)};
        ...
        public class DnsResourceRecord : IDnsResourceRecord
        {
            ...
            public byte[] ToArray()
            {
                return new MemoryStream(Size).Append(Name.ToArray()).Append(new Tail {Type = Type, Class = Class, TimeToLive = TimeToLive, DataLength = Data.Length}.ToBytes()).Append(Data).ToArray();
            }
            ...
            public override string ToString()
            {
                return Json.SerializeOnly(this, true, nameof(Name), nameof(Type), nameof(Class), nameof(TimeToLive), nameof(DataLength));
            }
        ...
        public class DnsCanonicalNameResourceRecord : DnsResourceRecordBase
        {
            ...
            protected override string[] IncludedProperties => new List<string>(base.IncludedProperties) {nameof(CanonicalDomainName)}.ToArray();
        }
        ...
        public class DnsMailExchangeResourceRecord : DnsResourceRecordBase
        {
            ...
            protected override string[] IncludedProperties => new List<string>(base.IncludedProperties) {nameof(Preference), nameof(ExchangeDomainName)}.ToArray();
        }
        ...
        public class DnsStartOfAuthorityResourceRecord : DnsResourceRecordBase
        {
            ...
            public DnsStartOfAuthorityResourceRecord(DnsDomain domain, DnsDomain master, DnsDomain responsible, long serial, TimeSpan refresh, TimeSpan retry, TimeSpan expire, TimeSpan minTtl, TimeSpan ttl = default)
            ...
            protected override string[] IncludedProperties => new List<string>(base.IncludedProperties) {nameof(MasterDomainName), nameof(ResponsibleDomainName), nameof(SerialNumber)}.ToArray();

            private static IDnsResourceRecord Create(DnsDomain domain, DnsDomain master, DnsDomain responsible, long serial, TimeSpan refresh, TimeSpan retry, TimeSpan expire, TimeSpan minTtl, TimeSpan ttl)
            {
                var data = new MemoryStream(Options.SIZE + master.Size + responsible.Size);
                var tail = new Options {SerialNumber = serial, RefreshInterval = refresh, RetryInterval = retry, ExpireInterval = expire, MinimumTimeToLive = minTtl};
