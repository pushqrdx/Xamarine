...
namespace Xamarine.Hosting.Swan.Networking
{
    ...
    ///     you will need to enable continuous reading and events. On the client side you may want to disable continuous
    ///     reading
    ...
    ///     Manual Reading Mode: Data reception events are NEVER fired. No background threads are used. Use Read methods to
    ///     receive data.
    /// </summary>
        ...
        public bool IsConnected
        {
            get
            {
                ...
                try
                {
                    ...
                    var pollResult = !(socket.Poll(1000, SelectMode.SelectRead) && NetworkStream.DataAvailable == false || !socket.Connected);
...
#region Read Methods
            ...
            try
            {
                ...
                while (receiveBuilder.Count <= 0)
                {
                    if (DateTime.UtcNow.Subtract(startTime) >= timeout)
                        throw new TimeoutException($"Reading data from {ActiveStream} timed out in {timeout.TotalMilliseconds} ms");
        ...
        public async Task<string> ReadLineAsync(TimeSpan timeout, CancellationToken ct = default)
        {
            ...
            while (true)
            {
                ...
                if (text.EndsWith(_newLineSequence) == false)
                    continue;

                var lines = builder.ToString().TrimEnd(_newLineSequenceChars).Split(_newLineSequenceLineSplitter, StringSplitOptions.None);
...
#region Write Methods
        ...
        {
            return WriteTextAsync(text, TextEncoding, ct);
        }
        ...
        {
            return WriteDataAsync(encoding.GetBytes(text), true, ct);
        }
        ...
        {
            return WriteLineAsync(line, TextEncoding, ct);
        }
...
#region Socket Methods
        ...
        public async Task<bool> UpgradeToSecureAsClientAsync(string hostname, RemoteCertificateValidationCallback callback)
        {
            if (IsActiveStreamSecure)
                return true;
        ...
        {
            return UpgradeToSecureAsClientAsync(Network.HostName.ToLowerInvariant(), (a, b, c, d) => true);
        }
...
#region Continuous Read Methods
        ...
        private void RaiseReceiveBufferEvents(byte[] receivedData)
        {
            ...
            var sequences = _receiveBuffer.Skip(0).Take(_receiveBufferPointer).ToArray().Split(0, _newLineSequenceBytes);
            ...
            for (var i = 0; i < sequences.Count; i++)
            {
                ...
                {
                    var eventArgs = new ConnectionDataReceivedEventArgs(sequenceBytes, ConnectionDataReceivedTrigger.NewLineSequenceEncountered, isLast == false);
                ...
                else
                {
                    ...
                    Array.Copy((Array) sequenceBytes, (Array) _receiveBuffer, (int) sequenceBytes.Length);
