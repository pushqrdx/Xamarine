...
#region Constructors
        ...
        public SmtpServerReply(int responseCode, string statusCode, params string[] content)
        {
            ...
            if (!IsValid)
                return;
            if (responseCode >= 200)
                ReplyCodeSeverity = SmtpReplyCodeSeverities.PositiveCompletion;
            if (responseCode >= 300)
                ReplyCodeSeverity = SmtpReplyCodeSeverities.PositiveIntermediate;
            if (responseCode >= 400)
                ReplyCodeSeverity = SmtpReplyCodeSeverities.TransientNegative;
            if (responseCode >= 500)
                ReplyCodeSeverity = SmtpReplyCodeSeverities.PermanentNegative;
            if (responseCode >= 600)
                ReplyCodeSeverity = SmtpReplyCodeSeverities.Unknown;
...
#region Methods
        ...
        public override string ToString()
        {
            ...
            for (var i = 0; i < Content.Count; i++)
            {
                ...
                builder.Append(isLastLine ? $"{responseCodeText}{statusCodeText} {Content[i]}" : $"{responseCodeText}-{Content[i]}\r\n");
            }
