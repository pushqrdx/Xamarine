...
namespace Xamarine.Hosting.Swan.Networking.Ldap
{
    ...
    internal class RfcFilter : Asn1Choice
    {
        ...
        public void AddSubstring(SubstringOp type, sbyte[] values)
        {
            try
            {
                ...
                if (type != SubstringOp.Initial && type != SubstringOp.Any && type != SubstringOp.Final)
                    throw new LdapException("Attempt to add an invalid substring type", LdapStatusCode.FilterError);

                if (type == SubstringOp.Initial && substringSeq.Size() != 0)
                    throw new LdapException("Attempt to add an initial substring match after the first substring", LdapStatusCode.FilterError);

                if (_finalFound)
                    throw new LdapException("Attempt to add a substring match after a final substring match", LdapStatusCode.FilterError);
        ...
        {
            Asn1Object current = new Asn1Tagged(new Asn1Identifier((int) FilterOp.Present), new Asn1OctetString(attrName), false);
            AddObject(current);
        }
            ...
            while (itr.MoveNext())
            {
                ...
                if (filterpart is int i)
                {
                    ...
                    switch (op)
                    {
                        ...
                        {
                            filter.Append((string) itr.Current).Append(">=").Append(Encoding.UTF8.GetString((sbyte[]) itr.Current));
                            break;
                        }
        ...
        private Asn1Tagged ParseFilterComp()
        {
            ...
            switch (filterComp)
            {
                case FilterOp.And:
                case FilterOp.Or:
                    tag = new Asn1Tagged(new Asn1Identifier((int) filterComp, true), ParseFilterList(), false);
                    break;
                case FilterOp.Not:
                    tag = new Asn1Tagged(new Asn1Identifier((int) filterComp, true), ParseFilter());
                    break;
                default:
                    ...
                    switch (filterType)
                    {
                        ...
                        case FilterOp.LessOrEqual:
                        case FilterOp.ApproxMatch:
                            tag = new Asn1Tagged(new Asn1Identifier((int) filterType, true), new RfcAttributeValueAssertion(_ft.Attr, UnescapeString(valueRenamed)), false);
                            break;
                        case FilterOp.EqualityMatch:
                            if (valueRenamed.Equals("*"))
                            {
                                // present
                                tag = new Asn1Tagged(new Asn1Identifier((int) FilterOp.Present), new Asn1OctetString(_ft.Attr), false);
                            }
                                ...
                                while (sub.HasMoreTokens())
                                {
                                    ...
                                    if (subTok.Equals("*"))
                                    {
                                        ...
                                        if (lastTok.Equals(subTok))
                                            seq.Add(new Asn1Tagged(new Asn1Identifier((int) SubstringOp.Any), UnescapeString(string.Empty)));
                                    }
                                    else
                                    {
                                        // value (RfcLdapString)
                                        if (cnt == 1)
                                            seq.Add(new Asn1Tagged(new Asn1Identifier((int) SubstringOp.Initial), UnescapeString(subTok)));
                                        else if (cnt < tokCnt)
                                            seq.Add(new Asn1Tagged(new Asn1Identifier((int) SubstringOp.Any), UnescapeString(subTok)));
                                        else
                                            seq.Add(new Asn1Tagged(new Asn1Identifier((int) SubstringOp.Final), UnescapeString(subTok)));
                                    }

                                    lastTok = subTok;
                                }

                                tag = new Asn1Tagged(new Asn1Identifier((int) FilterOp.Substrings, true), new RfcSubstringFilter(_ft.Attr, seq), false);
                            }
                            else
                            {
                                tag = new Asn1Tagged(new Asn1Identifier((int) FilterOp.EqualityMatch, true), new RfcAttributeValueAssertion(_ft.Attr, UnescapeString(valueRenamed)), false);
                            }
                        ...
                        case FilterOp.ExtensibleMatch:
                            ...
                            tag = new Asn1Tagged(new Asn1Identifier((int) FilterOp.ExtensibleMatch, true), new RfcMatchingRuleAssertion(matchingRule, type, UnescapeString(valueRenamed), attr == false ? null : new Asn1Boolean(true)), false);
            ...
            for (str = 0, octs = 0; str < length; str++)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        var escString = new StringBuilder();
                        ca[0] = ch;
                        ...
                        foreach (var u in Encoding.UTF8.GetSBytes(new string(ca)))
                        {
                            ...
                            escString.Append(Convert.ToString(u & 0xff, 16));
                        }
                ...
                switch (value)
                {
                    ...
                    default:
                        if (value.GetIdentifier().Tag == (int) FilterOp.Not)
                            throw new LdapException("Attemp to create more than one 'not' sub-filter", LdapStatusCode.FilterError);
                ...
                get
                {
                    ...
                    switch (testChar)
                    {
                        ...
                        default:
                            ...
                            while (delims.IndexOf(_filter[_offset]) == -1 && _filter.Substring(_offset).StartsWith(":=") == false)
            ...
            public object Current
            {
                get
                {
                    ...
                    else
                    {
                        ...
                        switch (asn1)
                        {
                            case Asn1OctetString s:

                                // one value to iterate
                                _hasMore = false;
                                toReturn = s.StringValue();
                                break;
                            case RfcSubstringFilter sub:
                                if (_index == -1)
                                {
                                ...
                                break;
                            case RfcAttributeValueAssertion assertion:
                                ...
                                break;
                            case RfcMatchingRuleAssertion exMatch:

                                // Extensible match
                                if (_index == -1)
                                    _index = 0;

                                toReturn = ((Asn1OctetString) ((Asn1Tagged) exMatch.Get(_index++)).TaggedValue).StringValue();
                                if (_index > 2)
                                    _hasMore = false;

                                break;
                            case Asn1SetOf setRenamed:

                                // AND and OR nested components
                                if (_index == -1)
                                    _index = 0;

                                toReturn = new FilterIterator(_enclosingInstance, (Asn1Tagged) setRenamed.Get(_index++));
