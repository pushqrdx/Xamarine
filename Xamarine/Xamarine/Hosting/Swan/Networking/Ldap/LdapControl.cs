...
namespace Xamarine.Hosting.Swan.Networking.Ldap
{
    ...
    public class LdapControl
    {
        ...
        {
            return Asn1Object.ControlValue?.ByteValue();
        }
    ...
    internal class LdapUrl
    {
        ...
        public override string ToString()
        {
            var url = new StringBuilder(256).Append(Secure ? "ldaps://" : "ldap://").Append(Host);

            // Port not specified
            if (_port != 0)
                url.Append(":" + _port);

            if (DN == null && AttributeArray == null && Scope == LdapScope.ScopeBase && Filter == null && Extensions == null)
                return url.ToString();
    ...
    internal class LdapResponse : LdapMessage
    {
        ...
        public LdapStatusCode ResultCode
        {
            get
            {
                if (Message.Response is RfcSearchResultEntry || (IRfcResponse) Message.Response is RfcIntermediateResponse)
            ...
            get
            {
                ...
                switch (ResultCode)
                {
                    ...
                    case LdapStatusCode.Referral:
                        var refs = Referrals;
                        ex = new LdapException("Automatic referral following not enabled", LdapStatusCode.Referral, ErrorMessage);
    ...
    internal sealed class RfcBindRequest : Asn1Sequence, IRfcRequest
    {
        ...
        {
            return ((Asn1OctetString) Get(1)).StringValue();
        }
