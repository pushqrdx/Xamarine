...
namespace Xamarine.Hosting.Swan.Networking.Ldap
{
    ...
    public sealed class LdapSearchResults
    {
        ...
        ///     The count.
        /// </value>
        public int Count => new List<RfcLdapMessage>(_messages).Count(x => x.MessageId == _messageId && GetResponse(x) is LdapSearchResult);
        ...
        public bool HasMore()
        {
            return new List<RfcLdapMessage>(_messages).Any(x => x.MessageId == _messageId && GetResponse(x) is LdapSearchResult);
        }
        ...
        {
            var list = new List<RfcLdapMessage>(_messages).Where(x => x.MessageId == _messageId);
