...
using Xamarine.Hosting.Swan.Networking;
...
#region IP Addresses and Adapters Information Methods
        ...
        public static Dictionary<NetworkInterface, IPInterfaceProperties> GetIPv4Interfaces()
        {
            ...
            var adapters = NetworkInterface.GetAllNetworkInterfaces()
                                           .Where(network => network.OperationalStatus == OperationalStatus.Up && network.NetworkInterfaceType != NetworkInterfaceType.Unknown &&
            ...
            foreach (var adapter in adapters)
            {
                ...
                if (properties == null || properties.GatewayAddresses.Count == 0 || properties.GatewayAddresses.All(gateway => Equals(gateway.Address, zeroConf)) || properties.UnicastAddresses.Count == 0 ||
                    properties.GatewayAddresses.All(address => Equals(address.Address, zeroConf)) || properties.UnicastAddresses.Any(a => a.Address.AddressFamily == AddressFamily.InterNetwork) == false)
                    continue;
...
#endif
                                                                                  (skipTypeFilter || ni.NetworkInterfaceType == interfaceType) && ni.OperationalStatus == OperationalStatus.Up).ToArray();
        ...
        {
            return GetIPv4Interfaces().Select(a => a.Value.DnsAddresses.Where(d => d.AddressFamily == AddressFamily.InterNetwork)).SelectMany(d => d).ToArray();
        }
...
#region DNS and NTP Clients
        ...
        public static Task<IPAddress[]> GetDnsHostEntryAsync(string fqdn, CancellationToken ct = default)
        {
        ...
        /// <param name="ct">The cancellation token.</param>
        /// <returns>An array of local ip addresses of the result produced by this task.</returns>
        public static Task<IPAddress[]> GetDnsHostEntryAsync(string fqdn, IPAddress dnsServer, int port, CancellationToken ct = default)
        {
        ...
        {
            return new DnsClient(dnsServer, port).Reverse(query);
        }
        ...
        /// <param name="ct">The cancellation token.</param>
        /// <returns>A <see cref="System.String" /> that represents the current object.</returns>
        public static Task<string> GetDnsPointerEntryAsync(IPAddress query, IPAddress dnsServer, int port, CancellationToken ct = default)
        {
        ...
        /// <param name="ct">The cancellation token.</param>
        /// <returns>A <see cref="System.String" /> that represents the current object.</returns>
        public static Task<string> GetDnsPointerEntryAsync(IPAddress query, CancellationToken ct = default)
        {
        ...
        public static Task<DnsQueryResult> QueryDnsAsync(string query, DnsRecordType recordType, IPAddress dnsServer, int port, CancellationToken ct = default)
        {
        ...
        /// <param name="ct">The cancellation token.</param>
        /// <returns>Queries the DNS server for the specified record type of the result produced by this Task.</returns>
        public static Task<DnsQueryResult> QueryDnsAsync(string query, DnsRecordType recordType, CancellationToken ct = default)
        {
        ...
        public static DateTime GetNetworkTimeUtc(string ntpServerName = "pool.ntp.org", int port = NtpDefaultPort)
        {
        ...
        public static Task<DateTime> GetNetworkTimeUtcAsync(IPAddress ntpServerAddress, int port = NtpDefaultPort, CancellationToken ct = default)
        {
        ...
        /// <param name="ct">The cancellation token.</param>
        /// <returns>The UTC time by querying from an NTP server of the result produced by this Task.</returns>
        public static Task<DateTime> GetNetworkTimeUtcAsync(string ntpServerName = "pool.ntp.org", int port = NtpDefaultPort, CancellationToken ct = default)
        {
