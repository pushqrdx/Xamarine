...
namespace Xamarine.Hosting.Swan
{
    ...
    {
        private static readonly Dictionary<string, int> DateRanges = new Dictionary<string, int> {{"minute", 59}, {"hour", 23}, {"dayOfMonth", 31}, {"month", 12}, {"dayOfWeek", 6}};
        ...
        {
            return DateTimeSpan.CompareDates(dateStart, dateEnd);
        }
        ...
        {
            var results = new List<bool?> {GetElementParts(minute, date.Minute), GetElementParts(hour, date.Hour), GetElementParts(dayOfMonth, date.Day), GetElementParts(month, date.Month), GetElementParts(dayOfWeek, (int) date.DayOfWeek)};
        ...
        private static bool? GetElementParts(int? status, int value)
        {
            return status.HasValue ? status.Value == value : (bool?) null;
        }
