...
namespace Xamarine.Hosting.Swan
{
    ...
    public static class ValueTypeExtensions
    {
        ...
        {
            return (uint) (((longBytes & 0x000000ff) << 24) + ((longBytes & 0x0000ff00) << 8) + ((longBytes & 0x00ff0000) >> 8) + ((longBytes & 0xff000000) >> 24));
        }
        ...
        private static byte[] GetStructBytes<T>(byte[] data)
        {
            ...
            foreach (var field in fields)
            {
                ...
                if (endian != null && (endian.Endianness == Endianness.Big && BitConverter.IsLittleEndian || endian.Endianness == Endianness.Little && !BitConverter.IsLittleEndian))
