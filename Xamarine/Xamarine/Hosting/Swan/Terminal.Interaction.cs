...
#region ReadKey
        ...
        /// <param name="disableLocking">
        ///     if set to <c>true</c> the output will continue to be shown.
        ///     This is useful for services and daemons that are running as console applications and wait for a key to exit the
        ///     program.
        /// </param>
        ...
        public static ConsoleKeyInfo ReadKey(this string prompt, bool preventEcho)
        {
            ...
            {
                if (prompt != null)
                    ($" {(string.IsNullOrWhiteSpace(Settings.LoggingTimeFormat) ? string.Empty : DateTime.Now.ToString(Settings.LoggingTimeFormat) + " ")}" + $"{Settings.UserInputPrefix} << {prompt} ").Write(ConsoleColor.White);
        ...
        public static ConsoleKeyInfo ReadPrompt(this string title, Dictionary<ConsoleKey, string> options, string anyKeyOption)
        {
            ...
            lock (SyncLock)
            {
...

                {
                    // Title
                    Table.Vertical();
                    var titleText = string.Format(textFormat, string.IsNullOrWhiteSpace(title) ? " Select an option from the list below." : $" {title}");
                ...
                {
                    Table.Vertical();
                    string.Format(textFormat, $"    {"[ " + kvp.Key + " ]",-10}  {kvp.Value}").Write(textColor);
                    Table.Vertical();
                }
                ...
                if (string.IsNullOrWhiteSpace(anyKeyOption) == false)
                {
                    ...
                    Table.Vertical();
                    string.Format(textFormat, $"    {" ",-10}  {anyKeyOption}").Write(ConsoleColor.Gray);
                    Table.Vertical();
                }

                {
                    ...
                    Table.Vertical();
                    string.Format(textFormat, Settings.UserOptionText).Write(ConsoleColor.Green);
                    Table.Vertical();
