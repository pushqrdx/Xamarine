...
#region Methods
        ...
        public void Read(int requestedBytes, byte[] target, int targetOffset)
        {
            lock (_syncLock)
            {
                if (requestedBytes > ReadableCount)
                    throw new InvalidOperationException($"Unable to read {requestedBytes} bytes. Only {ReadableCount} bytes are available");
        ...
        /// <exception cref="System.InvalidOperationException">
        ///     Unable to write to circular buffer. Call the Read method to make
        ...
        public void Write(IntPtr source, int length, TimeSpan writeTag)
        {
            lock (_syncLock)
            {
                if (ReadableCount + length > Length)
                    throw new InvalidOperationException($"Unable to write to circular buffer. Call the {nameof(Read)} method to make some additional room.");
