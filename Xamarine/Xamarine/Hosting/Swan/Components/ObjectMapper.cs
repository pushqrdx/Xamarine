...
namespace Xamarine.Hosting.Swan.Components
{
    ...
    public class ObjectMapper
    {
        ...
        ///     target.
        /// </exception>
        public static int Copy(object source, object target, string[] propertiesToCopy = null, string[] ignoreProperties = null)
        {
            if (source == null)
            ...
            return Copy(target, propertiesToCopy, ignoreProperties, GetSourceMap(source));
        }
        ...
        ///     target.
        /// </exception>
        public static int Copy(IDictionary<string, object> source, object target, string[] propertiesToCopy = null, string[] ignoreProperties = null)
        {
            if (source == null)
                throw new ArgumentNullException(nameof(source));
        ...
        ///     You can't map from type {source.GetType().Name} to
        ...
        public TDestination Map<TDestination>(object source, bool autoResolve = true)
        {
            ...
            var map = _maps.FirstOrDefault(x => x.SourceType == source.GetType() && x.DestinationType == typeof(TDestination));
        ...
        private static int Copy(object target, IEnumerable<string> propertiesToCopy, IEnumerable<string> ignoreProperties, Dictionary<string, TypeValuePair> sourceProperties)
        {
            // Filter properties
            ...
            var properties = Runtime.PropertyTypeCache.RetrieveFilteredProperties(target.GetType(), true, x => x.CanWrite).ToArray();

            return properties.Select(x => x.Name).Distinct().ToDictionary(x => x.ToLowerInvariant(), x => properties.First(y => y.Name == x)).Where(x => sourceProperties.Keys.Contains(x.Key))
                             .When(() => requiredProperties != null, q => q.Where(y => requiredProperties.Contains(y.Key))).When(() => ignoredProperties != null, q => q.Where(y => !ignoredProperties.Contains(y.Key)))
                             .ToDictionary(x => x.Value, x => sourceProperties[x.Key]).Sum(x => TrySetValue(x, target) ? 1 : 0);
        }
        ...
        private static void SetValue(KeyValuePair<PropertyInfo, TypeValuePair> property, object target)
        {
            ...
            {
                property.Key.SetValue(target, Enum.ToObject(property.Key.PropertyType, property.Value.Value));
            ...
            {
                property.Key.SetValue(target, Convert.ToBoolean(property.Value.Value));
            ...
            switch (source)
            {
                ...
                case IList sourceList when target is Array targetArray:
                    for (var i = 0; i < sourceList.Count; i++)
                        try
                        {
                            targetArray.SetValue(sourceList[i].GetType().IsValueType() ? sourceList[i] : sourceList[i].CopyPropertiesToNew<object>(), i);
                        }
                        catch
                        {
                            // ignored
                ...
                case IList sourceList when target is IList targetList:
                    var addMethod = targetType.GetMethods().FirstOrDefault(m => m.Name.Equals(Json.AddMethodName) && m.IsPublic && m.GetParameters().Length == 1);
        ...
        private static Dictionary<string, TypeValuePair> GetSourceMap(object source)
        {
            ...
            var sourceProperties = Runtime.PropertyTypeCache.RetrieveFilteredProperties(source.GetType(), true, x => x.CanRead).ToArray();

            return sourceProperties.Select(x => x.Name).Distinct()
                                   .ToDictionary(x => x.ToLowerInvariant(), x => new TypeValuePair(sourceProperties.First(y => y.Name == x).PropertyType, sourceProperties.First(y => y.Name == x).GetValue(source)));
        }
