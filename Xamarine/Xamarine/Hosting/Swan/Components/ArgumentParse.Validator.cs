...
namespace Xamarine.Hosting.Swan.Components
{
    ...
    public partial class ArgumentParser
    {
        private sealed class Validator
        {
            ...
            public bool IsValid()
            {
                return (_settings.IgnoreUnknownArguments || !UnknownList.Any()) && !RequiredList.Any();
            }

            public IEnumerable<ArgumentOptionAttribute> GetPropertiesOptions()
            {
                return _properties.Select(p => Runtime.AttributeCache.RetrieveOne<ArgumentOptionAttribute>(p)).Where(x => x != null);
            }
            ...
            private bool SetPropertyValue(PropertyInfo targetProperty, string propertyValueString, object result)
            {
                ...
                {
                    var parsedValue = Enum.Parse(targetProperty.PropertyType, propertyValueString, _settings.CaseInsensitiveEnumValues);
                ...
                return targetProperty.PropertyType.IsArray ? PopulateArray(targetProperty, propertyValueString, result) : targetProperty.TrySetBasicType(propertyValueString, result);
            }

            private static bool PopulateArray(PropertyInfo targetProperty, string propertyValueString, object result)
            {
                var optionAttr = Runtime.AttributeCache.RetrieveOne<ArgumentOptionAttribute>(targetProperty);
            ...
            {
                return _properties.FirstOrDefault(p => string.Equals(Runtime.AttributeCache.RetrieveOne<ArgumentOptionAttribute>(p)?.LongName, propertyName, _settings.NameComparer) ||
