...
namespace Xamarine.Hosting.Swan.Components
{
    ...
    public partial class ArgumentParser
    {
        ...
        private static void ReportUnknownVerb<T>()
        {
            ...
            Runtime.PropertyTypeCache.RetrieveAllProperties<T>(true).Select(x => Runtime.AttributeCache.RetrieveOne<VerbOptionAttribute>(x)).Where(x => x != null).Select(x => $"  {x.Name}\t\t{x.HelpText}").ToList()
