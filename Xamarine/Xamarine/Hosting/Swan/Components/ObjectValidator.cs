...
namespace Xamarine.Hosting.Swan.Components
{
    ...
    /// }
    /// </code>
    ///     The following code shows of to validate an object with a custom validator and some attributes using the Runtime
    ///     ObjectValidator singleton.
    ///     <code>
    /// using Unosquare.Swan.Components;
    /// 
    /// class Example
    ...
    {
        private readonly Dictionary<Type, List<Tuple<Delegate, string>>> _predicates = new Dictionary<Type, List<Tuple<Delegate, string>>>();
        ...
        ///     message.
        /// </exception>
        public void AddValidator<T>(Predicate<T> predicate, string message) where T : class
        {
            if (predicate == null)
