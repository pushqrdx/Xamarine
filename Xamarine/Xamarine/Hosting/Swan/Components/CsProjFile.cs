...
namespace Xamarine.Hosting.Swan.Components
{
    ...
    public class CsProjFile<T> : IDisposable where T : CsProjMetadataBase
    {
        ...
        {
            if (filename == null)
                filename = Directory.EnumerateFiles(Directory.GetCurrentDirectory(), "*.csproj", SearchOption.TopDirectoryOnly).FirstOrDefault();

            if (filename == null)
                filename = Directory.EnumerateFiles(Directory.GetCurrentDirectory(), "*.fsproj", SearchOption.TopDirectoryOnly).FirstOrDefault();
