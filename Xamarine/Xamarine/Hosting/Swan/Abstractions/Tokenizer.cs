...
namespace Xamarine.Hosting.Swan.Abstractions
{
    /// <summary>
    ...
    public abstract class Tokenizer
    {
        ...
        public virtual Operator[] GetDefaultOperators()
        {
            return new[]
            {
                new Operator {Name = "=", Precedence = 1}, new Operator {Name = "!=", Precedence = 1}, new Operator {Name = ">", Precedence = 2}, new Operator {Name = "<", Precedence = 2}, new Operator {Name = ">=", Precedence = 2},
                new Operator {Name = "<=", Precedence = 2}, new Operator {Name = "+", Precedence = 3}, new Operator {Name = "&", Precedence = 3}, new Operator {Name = "-", Precedence = 3}, new Operator {Name = "*", Precedence = 4},
                new Operator {Name = "/", Precedence = 4}, new Operator {Name = "\\", Precedence = 4}, new Operator {Name = "^", Precedence = 4}
            };
        }
        ...
        private void Tokenize(string input)
        {
            ...
            for (var i = startIndex; i < input.Length; i++)
            {
                ...
                if (char.IsNumber(input, i) || input[i] == NegativeChar && (Tokens.Any() && Tokens.Last().Type != TokenType.Number || !Tokens.Any()))
                {
        ...
        private int ExtractData(string input, int i, Func<string, TokenType> tokenTypeEvaluation, Func<char, bool> evaluation, int right = 0, int left = -1)
        {
            var charCount = 0;
        ...
        {
            return ExtractData(input, i, x => TokenType.Operator, x => x == OpenFuncChar || x == CommaChar || x == PeriodChar || x == StringQuotedChar || char.IsWhiteSpace(x) || char.IsNumber(x));
        }

        private int ExtractFunctionOrMember(string input, int i)
        {
            return ExtractData(input, i, ResolveFunctionOrMemberType, x => x == OpenFuncChar || x == CloseFuncChar || x == CommaChar || char.IsWhiteSpace(x));
        }

        private int ExtractNumber(string input, int i)
        {
            return ExtractData(input, i, x => TokenType.Number, x => !char.IsNumber(x) && x != PeriodChar && x != NegativeChar);
        }
        ...
        {
            return CompareOperators(GetOperatorOrDefault(op1), GetOperatorOrDefault(op2));
        }

        private Operator GetOperatorOrDefault(string op)
        {
            return _operators.FirstOrDefault(x => x.Name == op) ?? new Operator {Name = op, Precedence = 0};
        }
    }
