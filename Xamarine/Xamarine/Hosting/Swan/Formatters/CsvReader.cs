...
#region Read Methods
        ...
        /// <summary>
        ///     Reads a line of CSV text, converting it into a dynamic object in which properties correspond to the names of the
        ///     headings.
        /// </summary>
        ...
        public void ReadObject<T>(IDictionary<string, string> map, ref T result)
        {
            lock (_syncLock)
            {
                ...
                var properties = TypeCache.RetrieveFilteredProperties(typeof(T), true, x => x.CanWrite && Definitions.BasicTypesInfo.ContainsKey(x.PropertyType));
                ...
                for (var i = 0; i < _headings.Length; i++)
                {
                    ...
                    if (map.ContainsKey(_headings[i]) == false && string.IsNullOrWhiteSpace(map[_headings[i]]) == false)
                        continue;
                    ...
                    properties.FirstOrDefault(p => p.Name.Equals(propertyName))?.TrySetBasicType(values[i], result);
                }
            }
        }
        ...
        /// <exception cref="System.InvalidOperationException">ReadHeadings.</exception>
        /// <exception cref="System.IO.EndOfStreamException">Cannot read past the end of the stream.</exception>
        public T ReadObject<T>(IDictionary<string, string> map) where T : new()
        {
...
#region Helpers
        ...
        public static IList<T> LoadRecords<T>(Stream stream) where T : new()
        {
        ...
        /// <returns>A generic collection of objects that can be individually accessed by index.</returns>
        public static IList<T> LoadRecords<T>(string filePath) where T : new()
        {
