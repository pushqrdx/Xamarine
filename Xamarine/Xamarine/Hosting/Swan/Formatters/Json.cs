...
#region Public API
        ...
        /// </code>
        /// </example>
        public static string Serialize(object obj, bool format = false, string typeSpecifier = null, bool includeNonPublic = false, string[] includedNames = null, string[] excludedNames = null)
        {
        ...
        /// </returns>
        /// <exception cref="ArgumentException">You need to provide an object or array.</exception>
        public static string Serialize(object obj, bool format, string typeSpecifier, bool includeNonPublic, string[] includedNames, string[] excludedNames, List<WeakReference> parentReferences)
        {
            if (obj != null && (obj is string || Definitions.AllBasicValueTypes.Contains(obj.GetType())))
                return SerializePrimitiveValue(obj);

            var options = new SerializerOptions(format, typeSpecifier, includedNames, GeExcludedNames(obj?.GetType(), excludedNames), includeNonPublic, parentReferences);
        ...
        {
            return Converter.FromJsonResult(Deserializer.DeserializeInternal(json), resultType, includeNonPublic);
        }
...
#region Private API
        ...
        private static string[] GeExcludedNames(Type type, string[] excludedNames)
        {
            ...
            if (!IgnoredPropertiesCache.ContainsKey(type))
                IgnoredPropertiesCache[type] = type.GetProperties().Where(x => x?.GetCustomAttribute<JsonPropertyAttribute>()?.Ignored == true).Select(x => x.Name).ToArray();
