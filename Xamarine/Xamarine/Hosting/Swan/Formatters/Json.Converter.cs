...
using System.Text;
using Xamarine.Hosting.Swan.Attributes;

namespace Xamarine.Hosting.Swan.Formatters
{
    /// <summary>
    ...
    public static partial class Json
    {
        ...
        {
            private static readonly Dictionary<MemberInfo, string> MemberInfoNameCache = new Dictionary<MemberInfo, string>();
            ...
            private Converter(object source, Type targetType, ref object targetInstance, bool includeNonPublic)
            {
            ...
            /// <summary>
            ///     Converts a json deserialized object (simple type, dictionary or list) to a new instance of the specified target
            ///     type.
            /// </summary>
            ...
            internal static object FromJsonResult(object source, Type targetType, bool includeNonPublic)
            {
                object nullRef = null;
            ...
            private static Type GetAddMethodParameterType(Type targetType)
            {
                if (!ListAddMethodCache.ContainsKey(targetType))
                    ListAddMethodCache[targetType] = targetType.GetMethods().FirstOrDefault(m => m.Name.Equals(AddMethodName) && m.IsPublic && m.GetParameters().Length == 1)?.GetParameters()[0].ParameterType;
            ...
            private static object GetSourcePropertyValue(Dictionary<string, object> sourceProperties, MemberInfo targetProperty)
            {
                if (!MemberInfoNameCache.ContainsKey(targetProperty))
                    MemberInfoNameCache[targetProperty] = targetProperty.GetCustomAttribute<JsonPropertyAttribute>()?.PropertyName ?? targetProperty.Name;

                var targetPropertyName = MemberInfoNameCache[targetProperty];

                return sourceProperties.ContainsKey(targetPropertyName) ? sourceProperties[targetPropertyName] : null;
            }
            ...
            private void ResolveObject(object source, ref object target)
            {
                switch (source)
                {
                    ...
                    default:
                        ...
                        if (!ReflectionExtensions.TryParseBasicType(_targetType, (string) sourceStringValue, out target))
            ...
            private void PopulateIList(List<object> objects, IList list)
            {
                ...
                foreach (var item in objects)
                    try
                    {
                        list.Add(FromJsonResult(item, parameterType, _includeNonPublic));
                    }
                    catch
                    {
                        // ignored
            ...
            private void PopulateArray(List<object> objects, Array array)
            {
                ...
                for (var i = 0; i < objects.Count; i++)
                    try
                    {
                        var targetItem = FromJsonResult(objects[i], elementType, _includeNonPublic);
            ...
            private void GetEnumValue(string sourceStringValue, ref object target)
            {
                ...
                if (enumType == null && _targetType.GetTypeInfo().IsEnum)
                    enumType = _targetType;
                if (enumType == null)
                    return;
            ...
            private void PopulateDictionary(Dictionary<string, object> sourceProperties, IDictionary targetDictionary)
            {
                ...
                var addMethod = _targetType.GetMethods().FirstOrDefault(m => m.Name.Equals(AddMethodName) && m.IsPublic && m.GetParameters().Length == 2);
                ...
                foreach (var sourceProperty in sourceProperties)
                    try
                    {
                        var targetEntryValue = FromJsonResult(sourceProperty.Value, targetEntryType, _includeNonPublic);
            ...
            private void PopulateProperties(Dictionary<string, object> sourceProperties)
            {
                ...
                foreach (var property in properties)
                {
                    ...
                    try
                    {
                        var currentPropertyValue = !property.PropertyType.IsArray ? property.GetCacheGetMethod(_includeNonPublic).Invoke(_target, null) : null;

                        var targetPropertyValue = FromJsonResult(sourcePropertyValue, property.PropertyType, ref currentPropertyValue, _includeNonPublic);
            ...
            private void PopulateFields(Dictionary<string, object> sourceProperties)
            {
                foreach (var field in FieldTypeCache.RetrieveAllFields(_targetType))
                {
                    ...
                    var targetPropertyValue = FromJsonResult(sourcePropertyValue, field.FieldType, _includeNonPublic);
