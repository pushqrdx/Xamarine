...
#region Helper Methods
            ...
            private static string Escape(string str, bool quoted)
            {
                ...
                if (quoted)
                    builder.Append(StringQuotedChar);
                Escape(str, builder);
                if (quoted)
                    builder.Append(StringQuotedChar);
                    ...
                    switch (currentChar)
                    {
                        ...
                        default:
                            if (currentChar < ' ')
                            {
                                ...
                                builder.Append("\\u").Append(escapeBytes[1].ToString("X").PadLeft(2, '0')).Append(escapeBytes[0].ToString("X").PadLeft(2, '0'));
                            }
                            else
                            {
            ...
            private Dictionary<string, object> CreateDictionary(Dictionary<string, MemberInfo> fields, string targetType, object target)
            {
                ...
                foreach (var field in fields)
                    ...
                    try
                    {
                        objectDictionary[field.Key] = field.Value is PropertyInfo property ? property.GetCacheGetMethod(_options.IncludeNonPublic).Invoke(target, null) : (field.Value as FieldInfo)?.GetValue(target);
                    }
                    catch
                    {
                        /* ignored */
            ...
            private string ResolveDictionary(IDictionary items, int depth)
            {
                ...
                foreach (DictionaryEntry entry in items)
                {
                    ...
                    _builder.Append(StringQuotedChar).Append(ValueSeparatorChar).Append(" ");
            ...
            {
                if (_options.Format == false || depth <= 0)
                    return;
