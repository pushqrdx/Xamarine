...
namespace Xamarine.Hosting.Swan.Formatters
{
    ...
    public partial class Json
    {
        ...
        {
            private static readonly Dictionary<Type, Dictionary<Tuple<string, string>, MemberInfo>> TypeCache = new Dictionary<Type, Dictionary<Tuple<string, string>, MemberInfo>>();
            ...
            public SerializerOptions(bool format, string typeSpecifier, string[] includeProperties, string[] excludeProperties = null, bool includeNonPublic = true, IReadOnlyCollection<WeakReference> parentReferences = null)
            {
            ...
            {
                return GetPropertiesCache(targetType).When(() => _includeProperties?.Length > 0, query => query.Where(p => _includeProperties.Contains(p.Key.Item1)))
                                                     .When(() => _excludeProperties?.Length > 0, query => query.Where(p => !_excludeProperties.Contains(p.Key.Item1))).ToDictionary(x => x.Key.Item2, x => x.Value);
            }
            ...
            private static Dictionary<Tuple<string, string>, MemberInfo> GetPropertiesCache(Type targetType)
            {
                ...
                TypeCache[targetType] = fields.ToDictionary(x => new Tuple<string, string>(x.Name, x.GetCustomAttribute<JsonPropertyAttribute>()?.PropertyName ?? x.Name), x => x);
