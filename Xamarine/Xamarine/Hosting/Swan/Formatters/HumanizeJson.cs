...
namespace Xamarine.Hosting.Swan.Formatters
{
    internal class HumanizeJson
    {
        ...
        {
            return _builder == null ? string.Empty : _builder.ToString().TrimEnd();
        }
            ...
            foreach (var kvp in objects)
            {
                ...
                switch (kvp.Value)
                {
                    case Dictionary<string, object> valueDictionary:
                        ...
                        {
                            writeOutput = true;
                            _builder.Append($"{_indentStr}{kvp.Key,-16}: object").AppendLine();
                        }
                    ...
                    case List<object> valueList:
                        ...
                        {
                            writeOutput = true;
                            _builder.Append($"{_indentStr}{kvp.Key,-16}: array[{valueList.Count}]").AppendLine();
                        }
            ...
            foreach (var value in objects)
            {
                ...
                switch (value)
                {
                    case Dictionary<string, object> valueDictionary:
                        ...
                        {
                            writeOutput = true;
                            _builder.Append($"{_indentStr}[{index}]: object").AppendLine();
                        }
                    ...
                    case List<object> valueList:
                        ...
                        {
                            writeOutput = true;
                            _builder.Append($"{_indentStr}[{index}]: array[{valueList.Count}]").AppendLine();
                        }
        ...
        private void AppendString()
        {
            ...
            if (stringValue.Length + _indentStr.Length > 96 || stringValue.IndexOf('\r') >= 0 || stringValue.IndexOf('\n') >= 0)
            {
                _builder.AppendLine();
                var stringLines = Enumerable.Select<string, string>(stringValue.ToLines(), l => l.Trim()).ToArray();
