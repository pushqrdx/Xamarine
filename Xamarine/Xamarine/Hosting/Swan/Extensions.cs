...
using Xamarine.Hosting.Swan.Components;
...
namespace Xamarine.Hosting.Swan
{
    ...
    public static partial class Extensions
    {
        ...
        public static int CopyPropertiesTo<T>(this T source, object target) where T : class
        {
        ...
        {
            return ObjectMapper.Copy(source, target, null, ignoreProperties);
        }
        ...
        /// <param name="target">The target.</param>
        /// <returns>Number of properties that was copied successful.</returns>
        public static int CopyOnlyPropertiesTo<T>(this T source, object target) where T : class
        {
        ...
        {
            return ObjectMapper.Copy(source, target, propertiesToCopy);
        }
        ...
        public static T DeepClone<T>(this T source, string[] ignoreProperties = null) where T : class
        {
        ...
        /// </returns>
        /// <exception cref="ArgumentNullException">source.</exception>
        public static T CopyOnlyPropertiesToNew<T>(this object source, string[] propertiesToCopy) where T : class
        {
            if (source == null)
                throw new ArgumentNullException(nameof(source));
        ...
        public static void Retry(this Action action, TimeSpan retryInterval = default, int retryCount = 3)
        {
            if (action == null)
        ...
        public static T Retry<T>(this Func<T> action, TimeSpan retryInterval = default, int retryCount = 3)
        {
            if (action == null)
                throw new ArgumentNullException(nameof(action));
        ...
        public static string[] GetCopyableProperties(this object obj)
        {
            ...
            return Runtime.PropertyTypeCache.RetrieveAllProperties(obj.GetType(), true).Select(x => new {x.Name, HasAttribute = x.GetCustomAttribute<CopyableAttribute>() != null}).Where(x => x.HasAttribute).Select(x => x.Name).ToArray();
        }
        ...
        {
            return Runtime.ObjectValidator.IsValid(obj);
        }

        internal static void CreateTarget(this object source, Type targetType, bool includeNonPublic, ref object target)
        {
            switch (source)
            {
                case string _:
