...
#region Output Context
        ...
        private class OutputContext
        {
            ...
            public OutputContext()
            {
                ...
                OutputWriters = IsConsolePresent ? TerminalWriters.StandardOutput : IsDebuggerAttached ? TerminalWriters.Diagnostics : TerminalWriters.None;
            }
        ...
        private static void DequeueOutputAsync()
        {
            ...
            using (var tickLock = new ManualResetEvent(false))
            {
                while (true)
                {
                    ...
                    {
                        if (OutputQueue.TryDequeue(out var context) == false)
                            continue;
