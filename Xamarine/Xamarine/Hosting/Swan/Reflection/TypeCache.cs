...
namespace Xamarine.Hosting.Swan.Reflection
{
    ...
    public abstract class TypeCache<T> : CollectionCacheRepository<Type, T> where T : MemberInfo
    {
        /// <summary>
        ...
        public T[] Retrieve<TOut>(Func<IEnumerable<T>> factory)
        {
            return Retrieve(typeof(TOut), factory);
        }
    }
    ...
    public class PropertyTypeCache : TypeCache<PropertyInfo>
    {
        ...
        {
            return Retrieve<T>(onlyPublic ? GetAllPublicPropertiesFunc(typeof(T)) : GetAllPropertiesFunc(typeof(T)));
        }
        ...
        {
            return Retrieve(type, onlyPublic ? GetAllPublicPropertiesFunc(type) : GetAllPropertiesFunc(type));
        }
        ...
        {
            return Retrieve(type, onlyPublic ? GetAllPublicPropertiesFunc(type, filter) : GetAllPropertiesFunc(type, filter));
        }

        private static Func<IEnumerable<PropertyInfo>> GetAllPropertiesFunc(Type type, Func<PropertyInfo, bool> filter = null)
        {
            return GetPropertiesFunc(type, BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance, filter);
        }

        private static Func<IEnumerable<PropertyInfo>> GetAllPublicPropertiesFunc(Type type, Func<PropertyInfo, bool> filter = null)
        {
            return GetPropertiesFunc(type, BindingFlags.Public | BindingFlags.Instance, filter);
        }
        ...
        private static Func<IEnumerable<PropertyInfo>> GetPropertiesFunc(Type type, BindingFlags flags, Func<PropertyInfo, bool> filter = null)
        {
            ...
            return () => type.GetProperties(flags).Where(filter ?? (p => p.CanRead || p.CanWrite));
        }
    }
    ...
    public class FieldTypeCache : TypeCache<FieldInfo>
    {
        ...
        {
            return Retrieve<T>(GetAllFieldsFunc(typeof(T)));
        }
        ...
        {
            return Retrieve(type, GetAllFieldsFunc(type));
        }
