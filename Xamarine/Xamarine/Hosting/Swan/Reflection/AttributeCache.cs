...
namespace Xamarine.Hosting.Swan.Reflection
{
    ...
    public class AttributeCache : CollectionCacheRepository<MemberInfo, object>
    {
        ...
        public object[] Retrieve<T>(MemberInfo member, bool inherit = false) where T : Attribute
        {
            if (member == null)
        ...
        public T RetrieveOne<T>(MemberInfo member, bool inherit = false) where T : Attribute
        {
        ...
        public Dictionary<PropertyInfo, object[]> Retrieve<T>(Type type, bool inherit = false) where T : Attribute
        {
            if (type == null)
                throw new ArgumentNullException(nameof(type));

            return PropertyTypeCache.RetrieveAllProperties(type, true).ToDictionary(x => x, x => Retrieve<T>(x, inherit));
        }
        ...
        public Dictionary<PropertyInfo, object[]> RetrieveFromType<T>(Type attributeType, bool inherit = false)
        {
            ...
            return PropertyTypeCache.RetrieveAllProperties<T>(true).ToDictionary(x => x, x => Retrieve(x, attributeType, inherit));
        }
    }
}
