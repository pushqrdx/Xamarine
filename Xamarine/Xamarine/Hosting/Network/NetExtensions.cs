    ...
    internal static class NetExtensions
    {
        ...
        internal static IEnumerable<string> SplitHeaderValue(this string value, params char[] separators)
        {
            ...
            for (var i = 0; i < len; i++)
            {
                ...
                else if (StringExtensions.Contains(seps, c))
                {
                    if (!quoted)
                    {
        ...
        {
            return value.All(c => c >= 0x20 && c < 0x7f && !StringExtensions.Contains(Tspecials, c));
        }
        ...
        internal static CookieCollection GetCookies(this NameValueCollection headers, bool response)
        {
            ...
            return headers != null && headers.AllKeys.Contains(name) ? CookieCollection.Parse(headers[name], response) : new CookieCollection();
        }
        ...
        {
            return collection[name]?.Split(Strings.CommaSplitChar).Any(val => val.Trim().Equals(value, StringComparison.OrdinalIgnoreCase)) == true;
        }
