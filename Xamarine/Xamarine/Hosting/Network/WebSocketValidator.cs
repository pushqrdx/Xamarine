...
namespace Xamarine.Hosting.Network
{
    internal class WebSocketValidator
    {
        ...
        internal static string CheckIfAvailable(WebSocketState state, bool connecting = false, bool open = true, bool closing = false, bool closed = false)
        {
            return !connecting && state == WebSocketState.Connecting || !open && state == WebSocketState.Open || !closing && state == WebSocketState.Closing || !closed && state == WebSocketState.Closed
        ...
        {
            return code == CloseStatusCode.NoStatus ? (!string.IsNullOrEmpty(reason) ? "NoStatus cannot have a reason." : null) :
                   code == CloseStatusCode.MandatoryExtension && !client ? "MandatoryExtension cannot be used by a server." :
                   code == CloseStatusCode.ServerError && client ? "ServerError cannot be used by a client." :
                   !string.IsNullOrEmpty(reason) && Encoding.UTF8.GetBytes(reason).Length > 123 ? "A reason has greater than the allowable max size." : null;
        }
        ...
        internal bool CheckIfAvailable(out string message, bool client, bool server, bool connecting, bool open, bool closing, bool closed = true)
        {
        ...
        internal bool CheckHandshakeRequest(WebSocketContext context, out string message)
        {
            ...
            if (!_webSocket.IgnoreExtensions && !string.IsNullOrWhiteSpace(headers["Sec-WebSocket-Extensions"]))
            {
                message = "Includes an invalid Sec-WebSocket-Extensions header.";
                return false;
            }
        ...
        internal void CheckReceivedFrame(WebSocketFrame frame)
        {
            ...
            if (_webSocket.InContinuation && frame.IsData)
                throw new WebSocketException(CloseStatusCode.ProtocolError, "A data frame has been received while receiving continuation frames.");

            if (frame.IsCompressed && _webSocket.Compression == CompressionMethod.None)
                throw new WebSocketException(CloseStatusCode.ProtocolError, "A compressed frame has been received without any agreement for it.");

            if (frame.Rsv2 == Rsv.On)
                throw new WebSocketException(CloseStatusCode.ProtocolError, "The RSV2 of a frame is non-zero without any negotiation for it.");

            if (frame.Rsv3 == Rsv.On)
                throw new WebSocketException(CloseStatusCode.ProtocolError, "The RSV3 of a frame is non-zero without any negotiation for it.");
        }
        ...
        internal bool ValidateSecWebSocketExtensionsServerHeader(string value)
        {
            ...
            foreach (var e in value.SplitHeaderValue(Strings.CommaSplitChar))
            {
                ...
                if (comp && ext.IsCompressionExtension(_webSocket.Compression))
                {
                    ...
                    var invalid = ext.SplitHeaderValue(';').Any(t =>
                    {
                        t = t.Trim();
        ...
        {
            return value != null && value == WebSocket.Version;
        }
