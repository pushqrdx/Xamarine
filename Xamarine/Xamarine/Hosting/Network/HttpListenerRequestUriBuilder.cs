...
namespace Xamarine.Hosting.Network
{
    ...
    internal sealed class HttpListenerRequestUriBuilder
    {
        ...
        public static Uri GetRequestUri(string rawUri, Uri infoUri)
        {
            return new HttpListenerRequestUriBuilder(rawUri, infoUri.Scheme, infoUri.Authority, infoUri.LocalPath, infoUri.Query).Build();
        }
        ...
        private Uri Build()
        {
            ...
            if (_requestUri == null)
                Uri.TryCreate(_cookedUriScheme + SchemeDelimiter + _cookedUriHost + _cookedUriPath + _cookedUriQuery, UriKind.Absolute, out _requestUri);
        ...
        private void BuildRequestUriUsingRawPath()
        {
            ...
            if (_rawPath == string.Empty)
            {
                ...
                Uri.TryCreate(_cookedUriScheme + SchemeDelimiter + _cookedUriHost + path + _cookedUriQuery, UriKind.Absolute, out _requestUri);
            }
            else
            {
        ...
        private ParsingResult BuildRequestUriUsingRawPath(Encoding encoding)
        {
            ...
            _requestUriString = new StringBuilder().Append(_cookedUriScheme).Append(SchemeDelimiter).Append(_cookedUriHost);
        ...
        private bool EmptyDecodeAndAppendRawOctetsList(Encoding encoding)
        {
            ...
            try
            {
                ...
                AppendOctetsPercentEncoded(_requestUriString, Equals(encoding, Utf8Encoding) ? _rawOctets.ToArray() : Utf8Encoding.GetBytes(decodedString));
