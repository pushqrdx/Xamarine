...
namespace Xamarine.Hosting.Network
{
    ...
    public class CookieCollection : ICollection
    {
        ...
        {
            return new List<string>(value.SplitHeaderValue(Strings.CookieSplitChars)).ToArray();
        }
        ...
        private static int CompareCookieWithinSorted(Cookie x, Cookie y)
        {
            ...
            return ret != 0 ? ret : (ret = string.Compare(x.Name, y.Name, StringComparison.Ordinal)) != 0 ? ret : y.Path.Length - x.Path.Length;
        }
        ...
        private static CookieCollection ParseRequest(string value)
        {
            ...
            foreach (var t in pairs)
            {
                ...
                {
                    cookie.Port = pair.Equals("$port", StringComparison.OrdinalIgnoreCase) ? "\"\"" : GetValue(pair);
                }
                else
                {
                    if (cookie != null)
        ...
        private static CookieCollection ParseResponse(string value)
        {
            ...
            for (var i = 0; i < pairs.Length; i++)
            {
                ...
                else if (pair.StartsWith("expires", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    if (!DateTime.TryParseExact(buff.ToString(), new[] {"ddd, dd'-'MMM'-'yyyy HH':'mm':'ss 'GMT'", "r"}, new CultureInfo("en-US"), DateTimeStyles.AdjustToUniversal | DateTimeStyles.AssumeUniversal, out var expires))
        ...
        private int SearchCookie(Cookie cookie)
        {
            ...
            {
                var c = _list[i];
                if (c.Name.Equals(name, StringComparison.OrdinalIgnoreCase) && c.Path.Equals(path, StringComparison.OrdinalIgnoreCase) && c.Domain.Equals(domain, StringComparison.OrdinalIgnoreCase) && c.Version == ver)
                    return i;
            }
