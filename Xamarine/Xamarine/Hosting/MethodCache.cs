...
namespace Xamarine.Hosting
{
    internal class MethodCache
    {
        ...
        public MethodCache(MethodInfo methodInfo)
        {
            ...
            SetDefaultHeadersMethodInfo = methodInfo.DeclaringType?.GetMethod(nameof(WebApiController.SetDefaultHeaders));
            IsTask = methodInfo.ReturnType == typeof(Task<bool>);
            AdditionalParameters = methodInfo.GetParameters().Skip(2).Select(x => new AddtionalParameterInfo(x)).ToList();
        ...
        private static Delegate BuildDelegate(MethodInfo methodInfo, bool isAsync)
        {
            ...
            var argumentExpressions = methodInfo.GetParameters().Select((parameterInfo, i) => Expression.Convert(Expression.ArrayIndex(argumentsExpression, Expression.Constant(i)), parameterInfo.ParameterType)).Cast<Expression>().ToList();

            var callExpression = Expression.Call(Expression.Convert(instanceExpression, methodInfo.DeclaringType ?? throw new InvalidOperationException()), methodInfo, argumentExpressions);

            if (isAsync)
                return Expression.Lambda<AsyncDelegate>(Expression.Convert(callExpression, typeof(Task<bool>)), instanceExpression, argumentsExpression).Compile();

            return Expression.Lambda<SyncDelegate>(Expression.Convert(callExpression, typeof(bool)), instanceExpression, argumentsExpression).Compile();
        }
    }
    ...
    internal class MethodCacheInstance
    {
        ...
        public void ParseArguments(Dictionary<string, object> parameters, object[] arguments)
        {
            ...
            for (var i = 0; i < MethodCache.AdditionalParameters.Count; i++)
            {
                ...
                arguments[i + 2] = parameters.ContainsKey(param.Info.Name) ? param.GetValue((string) parameters[param.Info.Name]) : param.Default;
            }
        }
        ...
        public Task<bool> Invoke(object[] arguments)
        {
            ...
            return MethodCache.IsTask ? MethodCache.AsyncInvoke(controller, arguments) : Task.FromResult(MethodCache.SyncInvoke(controller, arguments));
        }
