...
namespace Xamarine.Hosting
{
    ...
    public static partial class Extensions
    {
        ...
        {
            return val.ReplaceAll(string.Empty, '{', '}', '?');
        }

        internal static Uri ToUri(this string uriString)
        {
            Uri.TryCreate(uriString, uriString.MaybeUri() ? UriKind.Absolute : UriKind.Relative, out var ret);
...
#region Constants
        ...
        private const string _regexRouteReplace = "([^//]*)";
        private const string _wildcardRouteReplace = "(.*)";
        private static readonly byte[] _lastByte = {0x00};
        private static readonly Regex _routeOptionalParamRegex = new Regex(@"\{[^\/]*\?\}", RegexOptions.IgnoreCase | RegexOptions.Compiled);
        private static readonly Regex _routeParamRegex = new Regex(@"\{[^\/]*\}", RegexOptions.IgnoreCase | RegexOptions.Compiled);
...
#region HTTP Request Helpers
        ...
        {
            var match = new Regex(basePath.Replace("*", _wildcardRouteReplace)).Match(requestPath);
        ...
        {
            return RequestRegexUrlParams(context.RequestUri.LocalPath, urlPattern);
        }
        ...
        /// </returns>
        public static Dictionary<string, object> RequestRegexUrlParams(this string requestPath, string basePath, Func<bool> validateFunc = null)
        {
            if (validateFunc == null)
                validateFunc = () => false;
            if (requestPath == basePath && !validateFunc())
                return new Dictionary<string, object>();

            var regex = new Regex(string.Concat("^", _routeParamRegex.Replace(basePath, _regexRouteReplace), "$"), RegexOptions.IgnoreCase);
            ...
            {
                var optionalPath = _routeOptionalParamRegex.Replace(basePath, string.Empty);
                ...
                if (optionalPath == tempPath)
                    return pathParts.Where(x => x.StartsWith("{")).ToDictionary(x => x.CleanParamId(), x => (object) null);
            }
            else
            {
                var i = 1; // match group index

                return pathParts.Where(x => x.StartsWith("{")).ToDictionary(x => x.CleanParamId(), x => (object) match.Groups[i++].Value);
            }
...
#region JSON and Exception Extensions
        ...
        public static bool JsonExceptionResponse(this HttpListenerContext context, Exception ex, HttpStatusCode statusCode = HttpStatusCode.InternalServerError)
        {
            context.Response.StatusCode = (int) statusCode;
                ...
                case CompressionMethod.Deflate:
                    if (mode == CompressionMode.Compress)
                        using (var compressor = new DeflateStream(targetStream, CompressionMode.Compress, true))
                        {
                            ...
                            // WebSocket use this
                            targetStream.Write(_lastByte, 0, 1);
